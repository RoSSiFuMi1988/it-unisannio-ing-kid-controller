		...
		if(stato.equalsIgnoreCase("disattivato"));
				...
				if(dist1<distanza){
					if(notifica.getType().equalsIgnoreCase("email")){	// EMAIL
						...
						request.setAttribute("text", "Il dispositovo � entrato nell'area impostata.");
				...
				if(distanza<dist1){
					if(notifica.getType().equalsIgnoreCase("email")){	// EMAIL
						request.setAttribute("to", notifica.getAddr());
						request.setAttribute("state", "Alert");
						request.setAttribute("text", "Il dispositivo � uscito dall'area impostata.");
						String address="SendMail";
						RequestDispatcher dispatcher = request.getRequestDispatcher(address);
						dispatcher.forward(request, response);
						//System.out.println("DISPATCHER "+notifica.getAddr()+" "+stato);
				...
				if(dist1<distanza){
					if(notifica.getType().equalsIgnoreCase("email")){	// EMAIL
						request.setAttribute("to", notifica.getAddr());
						request.setAttribute("state", "Alert");
						request.setAttribute("text", "Il dispositivo � rientrato nell'area impostata.");
						String address="SendMail";
						RequestDispatcher dispatcher = request.getRequestDispatcher(address);
						dispatcher.forward(request, response);
						//System.out.println("DISPATCHER "+notifica.getAddr()+" "+stato);
