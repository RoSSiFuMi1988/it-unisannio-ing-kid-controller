...
function load() {
	...
	if (GBrowserIsCompatible()) {
		...
		var begin = function(){
			GDownloadUrl("ppp.php?imei="+val, function(data, responseCode) {
			...
			for (var i = 0; i < markers.length; i++) {
				...
				var lon1 = parseFloat(markers[i].getAttribute("lon1"));
				var raggio = parseFloat(markers[i].getAttribute("raggio1"));

				alert("Latitudine: "+lat + " Longitudine: "+ lon +" lat1: "+ lat1 +" lon1: "+ lon1 +" raggio: "+raggio); 
				punto = new GLatLng(lat,lon);
				var pIniziale = new GLatLng(lat1,lon1);
				marker1 = new GMarker(punto, {title: "Ultima rilevazione: "+data});
				map.addOverlay(marker1);
				
				drawCircle(pIniziale, raggio);
